    ...
    Foundation.libs.tab = {
        ...
        toggle_active_tab: function(tab, location_hash) {
            var self = this, S = self.S, tabs = tab.closest("[" + this.attr_name() + "]"), tab_link = tab.find("a"), anchor = tab.children("a").first(), target_hash = "#" + anchor.attr("href").split("#")[1], target = S(target_hash), siblings = tab.siblings(), settings = tabs.data(this.attr_name(true) + "-init"), interpret_keyup_action = function(e) {
                ...
                var is_entry_location = window.location.href === self.entry_location, default_hash = settings.scroll_to_content ? self.default_tab_hashes[0] : is_entry_location ? window.location.hash : "fndtn-" + self.default_tab_hashes[0].replace("#", "");
    ...
    submitRepoForm: function(event) {
        ...
        success = function() {
            if (view.repoCollection.length === 0) {
                ...
                view.errorMessage = "";
                view.render();
                view.$("[data-repo-name-input]")[0].focus();
            }
...
gh.RepoView = Backbone.View.extend({
    ...
    render: function() {
        ...
        _.each(view.model.get("commits"), function(commit) {
            ...
            var commitView = new gh.CommitView({
                model: commit
            });
